  ...
  <section>
    ...
    <div :class="{ ma_fenetre: true, collapse: !showEtudiant }">
      <small class="form-text info-text">
       <font-awesome-icon :icon="['fas', 'info-circle']" />
        Cliquez sur un etudiant pour plus d'info.
    </small>
      <table class="table text-center table-sm table-custom">
        ...
        <tbody>
          <tr v-for="stud in students" :key="stud.id" @click="goToStudentDetail(stud.id)">
            <td>{{ stud.nom }}</td>
            <td>{{ stud.prenom }}</td>
            <td>{{ stud.login }}</td>
          </tr>
    ...
    <div :class="{ ma_fenetre: true, collapse: !showPromotion }">
      <ul v-for="p in promo" :key="p.id" class="list-style-none">
        <li class="text-center">{{ p.nom }}</li>
      </ul>
    </div>

    <!-- Devoirs -->
    <div :class="{ ma_fenetre: true, collapse: !showDevoir }">
      <p v-if="assignements.length == 0" class="text-center">
        Aucun devoir
      </p>
      <table class="table text-center table-sm table-custom" v-else>
        ...
        <tbody>
          <tr v-for="assignement in assignements" :key="assignement.id">
            ...
            <td>{{ assignement.dateDebut | formatDate}} au</td>
  ...
  export default { 
    ...
    methods: {
      ...
      goToStudentDetail(id) {
       const routeData = this.$router.resolve({name:'referent_etudiant_detail',params:{id:id}});
