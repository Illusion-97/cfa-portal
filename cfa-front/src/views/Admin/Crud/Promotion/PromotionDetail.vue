<template>
  <div class="container-fluid">
    ...
    <a class="float-right" href="#" id="navbardrop" data-toggle="dropdown">
      <font-awesome-icon :icon="['fas', 'sliders-h']" class="icon text-dark" />
    </a>
    <div class="dropdown-menu dropleft rounded-0 float-right">
      <!-- <router-link :to="{ name: 'modifier-formation', params: { id: 'fr' } }" class="icon-link dropdown-item">
        Modifier
      </router-link> -->
      <span class="icon-link dropdown-item" @click="updatePromo">
        Modifier
      </span>
    </div>
    ...
    <div class="row">
      <div class="col-md-3" align="center">
        <div class="identite">
          ...
          <p>{{ promotion.dateDebut | formatDate}} - {{ promotion.dateFin  | formatDate}}</p>
          <p>{{ ville }}</p>
        </div>
      </div>
    ...
    <div :class="{ ma_fenetre: true, collapse: !isEtudiant }">
      <table class="table">
        ...
        <tbody>
          <tr v-for="etudiant in promotion.etudiantsDto" :key="etudiant.id" @dblclick="clickEtudiant(etudiant)"
            class="mon-tr">
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="intervention in promotion.interventionsDto" :key="intervention.id"
            @dblclick="clickIntervention(intervention)" class="mon-tr">
...
<script>
  ...
  import {
    promotionApi
  } from "@/_api/promotion.api.js";
  import {
    centreFormationApi
  } from "@/_api/centreFormation.api.js";
  export default {
    name: "PromoDetail",
    components: {
      BodyTitle,
